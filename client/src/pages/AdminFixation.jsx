...
export default function AdminFinition({ API }) {
  ...
  const load = async () => {
    ...
    setError("");
    try {
      const res = await fetch(`${API}/api/fixations`);
  ...
  const rename = async (row) => {
    ...
    setError("");
    try {
      const res = await fetch(`${API}/api/fixations/${row._id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  const remove = async (row) => {
    ...
    setError("");
    try {
      const res = await fetch(`${API}/api/fixations/${row._id}`, { method: "DELETE" });
      if (!res.ok) throw new Error(await res.text());
  ...
  return (
    <div className="card">
      ...
      <div className="page-actions" style={{ flexWrap: "wrap", marginBottom: 12 }}>
        <input className="input" placeholder="Nom (ex: Mat / Brillant)" value={name} onChange={(e) => setName(e.target.value)} />
        ...
        <div className="table-wrap">
          <table className="table">
            ...
            <tbody>
                ...
                <tr key={row._id}>
                  ...
                  <td>
                    <div className="actions">
                      <button className="btn btn-outline" type="button" onClick={() => rename(row)}>Renommer</button>
                      <button className={`btn ${row.isActive ? "btn-dark" : "btn-outline"}`} type="button" onClick={() => toggle(row)}>
              ...
              {rows.length === 0 ? (
                <tr><td colSpan={3} className="muted">Aucune finition.</td></tr>
              ) : null}
            </tbody>
          </table>
        </div>
